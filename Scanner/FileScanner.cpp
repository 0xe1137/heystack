...
void FileScanner::start(const std::string &root_path) {
    if (const RecordId scan_root_id = index.addRecord(root_path, ROOT_ID, 0, FileFlags::Dir)) {
...
void FileScanner::scanDirectory(const std::string &path, RecordId parent_id) {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (entry->d_type == DT_DIR) {
            flags |= FileFlags::Dir;
            const RecordId id = index.addRecord(name, parent_id, 0, flags);

            scanDirectory(path + "/" + entry->d_name, id);
        } else if (entry->d_type == DT_REG) {
            flags |= FileFlags::File;
            index.addRecord(name, parent_id, 0, flags);
